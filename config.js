module.exports = {
    api: {
        url: 'https://api.drijonh.repl.co',
        path: '/'
    },
    user: {
        id: "778557320123777026",
        username: "</> Drijon",
        discriminator: "7577",
        nickname: "Drijon H.",
        nickavatar: "https://us-east-1.tixte.net/uploads/cdn.drijon.tk/chrome_bMJm0pToow.png",
        status: {
            state: {
                text: "online",
                color: "#0EEFD5"
            },
            emote: "https://cdn.discordapp.com/emojis/819816473383665724.webp?size=100&quality=lossless",
            text: "~ help@drijon.xyz ~ drijon.xyz"
        },
        activities: [],
        createdTimestamp: 1615924806867,
        avatar: "https://us-east-1.tixte.net/uploads/cdn.drijon.tk/chrome_bMJm0pToow.png",
        banner: "https://cdn.discordapp.com/attachments/957388729830486116/957974761722478732/IMG_2282.webp",
        accentColor: "#0EEFD5"
    },
    cards: [{
        name: 'Drijon | Chill & Fun 🪐',
        icon: 'https://cdn.discordapp.com/icons/840635474011226153/a_bc27249551f8062278cabe6faa38283b.gif?size=4096',
        description: 'My Server x)',
        url: 'discord.gg/Hfr2wdbGZC',
        position: `Owner`
    }, {
        name: 'PsychoCord | Gaming & Chill',
        icon: 'https://cdn.discordapp.com/icons/846059846033211402/f475dcf7565fbb27992bc901d2d5adf5.png?size=4096',
        description: 'German Gaming & Chill Server ',
        url: 'https://discord.gg/psychocord',
        position: `Sr. Moderator`
    }, {
        name: 'Spotify',
        icon: 'https://cdn.discordapp.com/emojis/940644008840032316.webp?size=100&quality=lossless',
        description: 'My Spotify Playlist',
        url: 'https://open.spotify.com/playlist/1Ex3MxbbKLI84k7LP1AYLE?si=4d3a5fdd17714dc9',
        position: `Playlist Owner`
    }, {
        name: 'E-Mail ',
        icon: 'https://cdn.discordapp.com/emojis/866089515536744468.webp?size=100&quality=lossless',
        description: 'My E-Mail to contact me ',
        position: `E-Mail`,
        url: 'help@drijon.xyz'
    }],
    header: {
        icon: `https://cdn.discordapp.com/avatars/778557320123777026/b5a47116b4f465d7de313b4ca9a50895.png?size=4096`,
        name: `Drijon H.`,
        buttons: {
            Discord: `https://discord.gg/Hfr2wdbGZC `,
            GitHub: `https://github.com/drijon7577`,
            Reddit: `https://reddit.com/drijonh_`
        }
    },
    metadata: {
        title: `Drijon`,
        description: `👋 Heyya, i'm Drijon [He/Him] | I live in Italy, and i am a Web Developer wanna know more about me? click the link below!`,
        image: `https://cdn.discordapp.com/avatars/778557320123777026/b5a47116b4f465d7de313b4ca9a50895.png?size=4096`,
        domainName: `https://drijon.xyz`,
        color: `#0EEFD5`,
        keywords: [
            'Drijon',
            'Drijon',
            'Drijon#7577',
            'Drijoncheen',
        ],

    }
}
